...
public class Laser extends Enemy {
	...
	public Laser(float x, float y) {
		super(x, y, POINTS, SPEED, DAMAGE, MAX_HEALTH, SIZE, "res/laserShip.png");
	}
	...
	public void update(float delta) {
		...
		if (distanceBetween(GameScreen.getPlayer()) > DISTANCE) {
			...
			if (fireTimer > FIRING_DELAY) {
				fireTimer = 0;
				GameScreen.addEntity(new Ball(getCenterX(), getCenterY(), getRotation() - 90)); //90 to offset image rotation
			}
		}
	}
